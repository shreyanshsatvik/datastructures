...
Node* const_tree(vector<int> &pre_order,int& i,int bound)
{
    if(pre_order[i]>bound || i==pre_order.size())
    {
        return NULL;
    }
    Node* new_node = create_node(pre_order[i++]);
    new_node->left = const_tree(pre_order,i,new_node->data);
    new_node->right = const_tree(pre_order,i,bound);
    return new_node;
...
{
    //code here
    vector<int> pre_order(pre,pre+size);
    int i = 0;
    return const_tree(pre_order,i,INT_MAX);
